...
class apiPagoflash
{
  ...
  function __construct($p_key_token, $p_key_secret, $p_url_punto_venta, $p_modo_prueba = FALSE)
  {
    ...
    {
      $this->_dominio_base = 'https://api.pagoflash.com/api.php';  
  ...
  private function generarSolicitudPOSTPagoFlash($p_datos, $p_navegador)
  {
    ...
    if(false == curl_setopt($v_curl, CURLOPT_VERBOSE, 1)){ $this->_codigo_error = self::ERROR_OPC_VERBOSE; }
    if(false == curl_setopt($v_curl, CURLOPT_USERAGENT, $p_navegador)){ $this->_codigo_error = self::ERROR_OPC_USERAGENT; }
    if(false == curl_setopt($v_curl, CURLOPT_SSL_VERIFYPEER, FALSE)){ $this->_codigo_error = self::ERROR_OPC_SSL; }
    if(false == curl_setopt($v_curl, CURLOPT_TIMEOUT, 30)){ $this->_codigo_error = self::ERROR_OPC_TIMEOUT; }
    if(false == curl_setopt($v_curl, CURLOPT_URL, $this->_dominio_base)){ $this->_codigo_error = self::ERROR_OPC_URL; }
    if(false == curl_setopt($v_curl, CURLOPT_RETURNTRANSFER, 1)){ $this->_codigo_error = self::ERROR_OPC_RETURN; }
    if(false == curl_setopt($v_curl, CURLOPT_POSTFIELDS, $p_datos)){ $this->_codigo_error = self::ERROR_OPC_POST; }
